...
function `A_sigma(w)` and the imaginary part of the retarded self-energy, with
`n_up`, `n_down` and `n_d` annotated on the plot. Where the two spins are
...
| | setup | what it shows |
|---|---|---|
| [01](01_half_filling.py) | `U=6`, `eps=-U/2`, flat-DOS bath, equilibrium | the particle-hole symmetric point: `n=1/2`, symmetric spectrum, sharp quasiparticle peak between two Hubbard bands |
| [02](02_near_half_filling.py) | `U=6`, `eps=-3.25`, same bath | slightly above half filling; the spectrum loses its symmetry but keeps the three-peak structure |
| [03](03_away_from_half_filling.py) | `U=6`, `eps=0`, same bath | far from half filling, where the Kajueter-Kotliar interpolation earns its keep; `n_d` collapses to 0.016 |
| [04](04_voltage_bias.py) | `U=6`, `eps=-3.25`, same bath, `V=1.5` | a genuine nonequilibrium steady state; compare `n_d` with 02 to see the bias suppress it |
| [05](05_spin_dependent_hybridization.py) | `U=3`, `eps=-U/2`, Lorentzians centred at `+1` (up) and `-1` (down) | a per-flavor bath, and the strong spin polarization it produces even with a spin-independent onsite energy in equilibrium |

Examples 01--04 use the flat-DOS ("box") reservoir with half-bandwidth `D=10`
and `t_l = t_r = 1/sqrt(2)`, so that `Gamma = -Im Delta^R(0) = 1`. That is the
...
(`n_up = 0.78`, `n_down = 0.22`). The two spins see *identical* broadening at
the Fermi level, `Gamma_up(0) = Gamma_down(0)`, so this is not a density-of-states
effect: it is driven entirely by the real part of the hybridization, which acts
as a static exchange field `Re Delta^R_sigma(0) = -/+ 0.5`. See the note on
