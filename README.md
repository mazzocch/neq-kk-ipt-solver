...
Given a hybridization function $\Delta(\omega)$ (retarded and Keldysh components)
and Hubbard interaction $U$, it solves a coupled 4-dimensional nonlinear root
problem (auxiliary chemical potential $\mu_0$ and occupation $n$, for each spin flavor)
...
[Phys. Rev. B 55, 16132 (1997)] to the self-energy. See
[`docs/theory/potthoff_band_shift.md`](docs/theory/potthoff_band_shift.md)
...
This README is the primary documentation. A Sphinx-built site in `docs/`
additionally provides a full API reference (every public method with its
rendered docstring and equations) and a dedicated page deriving the
Potthoff-Wegner-Nolting band-shift correction in full
(`docs/theory/potthoff_band_shift.md`).

It is not hosted online yet. To build and view it locally:

```bash
pip install -e ".[docs]"
cd docs
make html
open _build/html/index.html   # or: python3 -m http.server 8000 --directory _build/html
```
...
input_data = {
    ...
    "solver": {
        ...
        "modifiable": {},
        "dynamic": {
            # Box-shaped-DOS reservoir hybridization
            "T": 0.05,
            "T_fict": 0.5,
            "D_l": 10.0,
            "D_r": 10.0,
...
| `Delta_eta` | number or object | Constant broadening subtracted from $\mathrm{Im}\,\Delta^R$, removing the hard zeros outside the band. Optional, default `0`. See below. |
...
strength is $\Gamma = -\mathrm{Im}\,\Delta^R(\text{center}) = 2/D$ — a semicircle matching a
given $\Gamma$ needs `Delta_D` $= 2/\Gamma$. For any other normalization, pass
`Delta_R_im`/`Delta_K_im` directly.

`Delta_eta` exists because a bare semicircle has $\mathrm{Im}\,\Delta^R$ *exactly* zero
...
band spanning the whole grid, so it adds $2\,w_{\max}\eta/\pi$ to the hybridization
weight $D_1$ (about 19% at $\eta=0.01$, $w_{\max}=30$) and makes $D_1$ depend on
$w_{\max}$. $D_2$ is unaffected. As a guide, at `Delta_D`$=2$, $U=4$,
`N_points`$=10001$, $w_{\max}=30$: $\eta=0.001$ is not enough ($\int A\,d\omega$ still $0.998$),
$\eta=0.005$–$0.01$ restores $\int A\,d\omega$ to $0.99999$ and every sum rule. Keeping the
...
**4. Box-shaped-DOS reservoir** (fallback if none of the above is given):

| Key | Type | Description |
|---|---|---|
| `T_fict` | number | Fictitious inverse temperature shaping the box-shaped DOS. |
...
In practice, $w_{\max}$ needs to be large relative to $U$ and the
hybridization bandwidth for this decay to actually happen, and $N_{\text{points}}$
...
$\pm w_{\max}$ before trusting a result computed on a coarser grid.
...
that vanishes identically outside some window) has $\mathrm{Im}\,\Delta^R = 0$ beyond the
...
and `N_points`$=10001$, the upper Hubbard band sits near $\omega = +5.9$ outside the
band and $\int A(\omega)\,d\omega$ drops to $\sim 0.97$ instead of $1$, which then breaks every sum
rule. Checking $\int A(\omega)\,d\omega = 1$ is the fastest way to detect this.
...
resolvable width, at the cost of a $w_{\max}$-dependent shift in $D_1$; or refine
the grid until the peak is resolved (`N_points=80001` restores $\int A\,d\omega$ to
$0.99999$ in the case above, which is expensive). The box-shaped-DOS reservoir is
immune either way — its $T_{\text{fict}}$ edge smearing leaves $\mathrm{Im}\,\Delta^R$ nonzero
everywhere.

**The spectral moment sum rules are a much more sensitive grid diagnostic than
the occupations.** $n$ and `n_double` are $\omega^0$-weighted integrals of
integrands that are already negligible in the tails, and are correspondingly
insensitive to the grid: over a 4x range in $w_{\max}$ and an 8x range in
resolution they do not move in the sixth decimal. The moments carry an $\omega^m$
...
2. **Judges convergence by the residual, not by the optimizer's flag.** An
   attempt counts as converged only if $|\text{residual}| \le$ `residual_tol`. This is not
   ...
   a strongly polarized start it returns success with $|\text{residual}| \sim 10^{-2}$,
   against $\sim 10^{-12}$ for the genuine solution, and the $m=1$ sum rule is then off by
   $6\times 10^{-2}$ instead of $10^{-4}$. `sol.success` is overwritten to reflect the residual
...
The essentials; see [Documentation](#documentation) above for the full,
...
- `check_sum_rules(solver)` — per-flavor comparison of the closed-form $m=1,2,3$ moments against the numerically integrated $\int d\omega\, \omega^m A(\omega)$, plus the $m=0$ normalization.
...
a spin-dependent hybridization in equilibrium (Lorentzians centred at $\pm 1$,
...
normalization, and the exact $n_\uparrow + n_\downarrow = 1$ imposed by the combined
particle-hole $\times$ spin-flip symmetry of the spin-dependent setup.
...
and $m=2$ close on the occupations alone and hold to better than $10^{-4}$ for both
...
box-shaped-DOS reservoir at $V=1$, and by tens of percent for a spin-dependent
...
flat in both $w_{\max}$ and grid spacing, so it is a property of the ansatz and
